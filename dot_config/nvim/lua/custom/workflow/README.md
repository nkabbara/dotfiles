...
Use `:WorkspaceDelete!` to force deletion. Force mode still refuses protected branches, but skips the clean-worktree and merged-into checks, removes the worktree with `git worktree remove --force`, and deletes the branch with `git branch -D`.
